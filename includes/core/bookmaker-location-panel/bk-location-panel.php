...
if(!function_exists('aw_select_relate_bookmakers')):
  function aw_select_relate_bookmakers($country_id, $params=["unique"=>false,"random"=>false,"on_page"=>false,"on_single"=>false, "limit"=>false]){
    ...
    $table2 = MYSQL_TABLE_BK_COUNTRY_RELATIONS;
    $query = "SELECT * FROM $table Where exists (
      select * from $table2 Where ".(isset($params["on_page"]) ? "$table2.on_page ='1' and " : '')." ".(isset($params["on_single"]) ? "$table2.on_single ='1' and " : '')." $table2.country_id = $country_id and $table.ID = $table2.bookmaker_id ) AND $table.post_type='bk' AND $table.post_status='publish' ".(isset($params["random"]) ? "ORDER BY RAND()" : '')." ".(isset($params["limit"]) ? "LIMIT {$params['limit']}" : '')." ";
