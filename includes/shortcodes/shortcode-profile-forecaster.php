...
function shortcode_profile_forecaster($atts)
{
    ...
    $args['paged_free']  = isset($_GET['page_free']) ? $_GET['page_free'] : 1 ;
    ...
    $get_params_free = "?author_id={$args["author_id"]}&post_type=forecast&paged={$args['paged_free']}";
    ...
    //TABLE FREE
        ...
        if(intval($data_json_free->max_pages) > 0):
            for($i=1;$i<=intval($data_json_free->max_pages=8);$i++):
                $a_elements_free .= '<a class="page-numbers '.($args['paged_free'] == $i ? 'current' : '').'" href="?profile='.$args["author_id"].'&page_free='.$i.'">'.$i.'</a>';
            endfor;
        endif;
...
function load_forecaster_styles() {
    ...
    if (isset($post) && is_a($post, 'WP_Post') && (has_shortcode($post->post_content, 'profile-forecaster'))) {
        ...
        // Registrar y encolar jQuery solo si no está registrado
        if (!wp_script_is('jquery', 'registered')) {
            wp_register_script('jquery', includes_url('/js/jquery/jquery.min.js'), array(), '3.6.0', true);
        }
        
        // Asegurarte de que jQuery se encole
