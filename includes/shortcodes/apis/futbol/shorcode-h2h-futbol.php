...
function findClosestMatch($needle, $haystack) {
    // Verifica si $haystack no es un array u objeto válido.
    if (!is_array($haystack) && !is_object($haystack)) {
        return null; // Otra opción podría ser generar un error o mensaje adecuado.
    }

    $closestMatch = null;
    $shortestDistance = PHP_INT_MAX;
    $threshold = 5; // Umbral para la distancia de edición.

    // Divide el nombre de la aguja en palabras.
    $needleWords = explode(' ', $needle);

    foreach ($haystack as $name) {
        // Divide el nombre del equipo en palabras.
        $nameWords = explode(' ', $name);

        // Inicializa la suma de distancias de edición para este nombre.
        $totalDistance = 0;

        // Normaliza los caracteres en ambos nombres.
        $needleNormalized = iconv('UTF-8', 'ASCII//TRANSLIT', $needle);
        $nameNormalized = iconv('UTF-8', 'ASCII//TRANSLIT', $name);

        // Divide los nombres normalizados en palabras.
        $needleWordsNormalized = explode(' ', $needleNormalized);
        $nameWordsNormalized = explode(' ', $nameNormalized);

        // Calcula la distancia de edición para cada palabra individual.
        foreach ($needleWordsNormalized as $needleWord) {
            $minWordDistance = PHP_INT_MAX;
            foreach ($nameWordsNormalized as $nameWord) {
                $distance = levenshtein($needleWord, $nameWord);
        ...
        if ($totalDistance < $shortestDistance && $totalDistance < $threshold) {
...
function equipo_info_shortcode($atts) {
    ...
    $cache_key = 'equipo_info_' . $current_post_id . '_' . $atts['team1'] . '_' . $atts['team2'];
...
if ($team1_name && $team2_name) {
    ...
    $api_url = "https://v3.football.api-sports.io/fixtures/headtohead?h2h=$team1_id-$team2_id";
...
// Verifica si la solicitud a la API fue exitosa y se obtuvieron los datos de los enfrentamientos.
    ...
    $recent_finished_matches = array_filter($h2h, function($match) {
        return $match['fixture']['status']['long'] === 'Match Finished';
    });
    ...
    $recent_finished_matches = array_slice($recent_finished_matches, 0, 5);
            ...
            foreach ($recent_finished_matches as $match) {
                ...
                $match_date = date('d-m-Y', $match['fixture']['timestamp']); // Convierte el timestamp en una fecha y hora legible.
...
add_shortcode('equipos_h2h', 'equipo_info_shortcode');